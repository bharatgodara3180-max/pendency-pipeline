...
def wait_for_page_settle(page):
    """Give the Angular app extra time to finish rendering."""
    page.wait_for_timeout(15000)


def download_report(page, save_as, attempts=3):
    """Clicks Apply & Download, waits for the file to be generated, and
    downloads it. Retries from a fresh page state if needed."""
    last_error = None

    for attempt_num in range(1, attempts + 1):
        try:
            # Wait for the download button to appear.
            download_button = page.locator("button.dwnld-btn").first
            download_button.wait_for(state="visible", timeout=60000)
            download_button.click(force=True)

            # REV can take longer to render this button.
            apply_button = page.get_by_role(
                "button",
                name="Apply & Download"
            ).first
            apply_button.wait_for(state="visible", timeout=60000)
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            try:
                ...
                page.screenshot(
                    path=os.path.join(SCREENSHOT_DIR, "fwd_failure.png"),
                    full_page=True
                )
            ...
            try:
                ...
                os.makedirs(SCREENSHOT_DIR, exist_ok=True)
                page.screenshot(
                    path=os.path.join(SCREENSHOT_DIR, "rev_failure.png"),
                    full_page=True
                )
            ...
            if not results["fwd"] and not results["rev"]:
                os.makedirs(SCREENSHOT_DIR, exist_ok=True)
                page.screenshot(
                    path=os.path.join(SCREENSHOT_DIR, "failure.png"),
                    full_page=True
                )
                raise RuntimeError(
                    "Both FWD and REV downloads failed -- nothing to push this run."
                )
